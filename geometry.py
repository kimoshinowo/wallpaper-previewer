...
def find_corners(hough_corners, harris_corners):
    ...
    if all_corners.size >= 0:
        ...
        clf = DBSCAN(eps=20, min_samples=max((all_corners.size/4), 200)).fit(all_corners.reshape(-1, 1))
            ...
            if len(convex_hull) == 4:
                ...
                for ind1 in range(0,4):
                    for ind2 in range(0,4):
                        if (abs(convex_hull[ind1][0] - convex_hull[ind2][0]) <= (0.2 * width)) and (convex_hull[ind1][0] != convex_hull[ind2][0]):
